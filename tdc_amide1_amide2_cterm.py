...
E0AmideI = 1752.56*0.93                 # unperturbed energy of mode 
#E0AmideI = 1673.76                 # fit to experiment
u0AmideI = np.array([0.8470922, 0.52837629, 0.05703772])
u0AmideI /= np.linalg.norm(u0AmideI)
dipoleStrengthAmideI = 0.374      # oscillator strength in Debye 

E0Cterm = 1659.95*0.93
#E0Cterm = 1573.55613  # fit to experiment
u0Cterm = np.array([-0.31096098, 0.89573944, 0.3177328])
u0Cterm /= np.linalg.norm(u0Cterm)
dipoleStrengthCterm = 1.74*dipoleStrengthAmideI

E0AmideII = 1527.27*0.93                 # unperturbed energy of mode 
#E0AmideII = 1453.99672                  # fit to experiment
u0AmideII = np.array([0.06595929,-0.98665255, 0.14888293])
u0AmideII /= np.linalg.norm(u0AmideII)
dipoleStrengthAmideII = 0.4*dipoleStrengthAmideI      # oscillator strength in Debye 
...
traj_file = "SolvatedShellsUnitCell7LayerBox_nowaterinpores_ff14ipq.run99_nowater.dcd"
top_file = "SolvatedShellsUnitCell7LayerBox_nowaterinpores_ff14ipq.stripwater.prmtop"
...
#    if ts.frame%10==0:
        ...
        for resid1 in range(nAmide):
            ...
            dipoleCoordAmideI[resid1,:] = (currentAmide.positions[0,:] + currentAmide.positions[1,:])/2.0   # halfway between carbon and oxygen
            dipoleCoordAmideII[resid1,:] = currentAmide.positions[2,:]   # nitrogen
...
f /= np.amax(f)
np.savetxt("crystal_amide1_cterm_amide2_com_irspectrum.txt",np.column_stack((x,f)))
